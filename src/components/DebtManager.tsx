...
export default function DebtManager() {
  ...
  const handlePayInstallment = async (e: React.FormEvent, debt: Debt) => {
     e.preventDefault();
     const user = auth.currentUser;
     if (!user || !installmentAmount) return;
          ...
          debts.map(debt => {
            ...
            <div key={debt.id} className={`p-4 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl relative ${debt.status === 'paid' ? 'opacity-60' : ''}`}>
              ...
              {payingDebtId === debt.id && (
                <form onSubmit={(e) => handlePayInstallment(e, debt)} className="mt-3 flex gap-2">
                  <input 
                    type="number" 
                    value={installmentAmount}
                    onChange={(e) => setInstallmentAmount(e.target.value)}
                    placeholder="Nominal cicilan..."
                    className="flex-1 text-xs px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:outline-indigo-500"
                    required
                  />
                  <button type="button" onClick={() => setPayingDebtId(null)} className="text-xs px-2 text-slate-500">Batal</button>
                  <button type="submit" className="text-xs bg-indigo-500 text-white px-3 rounded-lg font-medium">Bayar</button>
                </form>
              )}
            </div>
          );
